 ...
 * <p>
 * This manager ensures notifications are displayed one at a time without overlapping
 * with existing game notifications, and properly cleans up when the player logs out.
 */
@Slf4j
@Singleton
public class NotificationManager
{
	...
	/**
	 * Processes the next queued notification on each game tick if no notification is currently visible.
	 */
	@Subscribe
	...
	/**
	 * Shuts down the notification manager, clears pending notifications, and unregisters event listeners.
	 */
	...
	/**
	 * Processes the next notification in the queue if no notification is currently visible.
	 */
	 ...
	 *
	 * @return {@code true} if a notification is visible (prevents overlapping notifications)
	 */
	 ...
	 * <p>
	 * Opens the notification interface, runs the display script with the notification data,
	 ...
	 * <p>
	 * Polls the widget width until it reaches zero (indicating the slide-out animation finished),
